...
print(f"Welcome to Fastrack USSD Banking Project")
time.sleep(8)
...
def open_acct():
    global gen_bvn
    print(f"Welcome to our online Account opening services")
    print("loading")
# creating an empty list to serve as a temporary place holder
    ...
    def balance():
        ...
        print("press # to go backto the Main Menu.")
        pin=input("Enter your 4 digit pin:")
#isdigit() is used to check for digits within a str while nested if is used to make sure user input 4 digits.

### i am to put the pin trial in a while loop......##REMINDER!!!

        if len(pin)!=4:
            print("Make sure its a 4 digits pin.")
            time.sleep(5)
            balance()
        else:
            if pin.isdigit():
...
def funds():
    time.sleep(3)
    print(bank_list)
    bnk = input("Select recipients Bank:")
...
#This is the function that prompts the user as to whether the user wants to continueor stop transaction.
def exit():
...
#This is the function for logging in using the fast ussd code *919#
def log_in():
    try:
        a=0
        while a<3:
